...
const COUNTS: [u32; 10] = [6, 2, 5, 5, 4, 5, 6, 3, 7, 6];
const FREQUENCY: [u32; 7] = [8, 6, 8, 7, 4, 9, 7];
...
fn determine_digits(input: (Vec<&str>, Vec<&str>)) -> usize {
    let mut mapping: HashMap<char, char> = HashMap::new();
    for digit in CHARS {
        let frequency_dig = input
            .0
            .iter()
            .map(|x| x.chars().filter(|y| *y == digit).count())
            .sum();
        let mapped_letter = determine_letter(digit, frequency_dig, &input.0);
        mapping.insert(digit, mapped_letter);
    }

    // Take the left handside and replace each number by it's mapping
    //let new_numbers = vec![];
    let mut total = 0;
    for number in input.1.iter().rev().enumerate() {
        total += convert_number(&mapping, number.1) * 10_usize.pow(number.0 as u32);
    }

    total
}

fn convert_number(mapping: &HashMap<char, char>, number: &str) -> usize {
    // Map to correct chars
    let mut number_vec = vec![];
    for char in number.chars() {
        number_vec.push(mapping.get(&char).unwrap());
    }

    number_vec.sort();

    let chars: String = number_vec.into_iter().collect();

    for dig in NUMBERS.iter().enumerate() {
        if chars == String::from(*dig.1) {
            return dig.0;
        }
    }
...
fn determine_letter(letter: char, frequency: usize, inputs: &Vec<&str>) -> char {
    match frequency {
        4 => 'e',
        6 => 'b',
        7 => {
            // Find 4
            // If the letter is in there it's d else g
            let four = inputs.iter().filter(|x| x.len() == 4).nth(0).unwrap();
            if four.chars().any(|x| x == letter) {
                'd'
            } else {
                'g'
            }
        ...
        8 => {
            // Find 1
            // If the letter is in there then it's c else a
            let one = inputs.iter().filter(|x| x.len() == 2).nth(0).unwrap();
            if one.chars().any(|x| x == letter) {
                'c'
            } else {
                'a'
            }
...
pub fn day_08() -> Result<()> {
    ...
    let lines: Vec<(&str, &str)> = lines
        ...
        .map(|x| {
            let mut y = x.split("|");
            (y.nth(0).unwrap().trim(), y.nth(0).unwrap().trim())
        })
        .collect();

    let lines: Vec<(Vec<&str>, Vec<&str>)> = lines
        .iter()
        .map(|x| (x.0.split(" ").collect(), x.1.split(" ").collect()))
        .collect();
    ...
    let count: usize = lines.into_iter().map(|x| determine_digits(x)).sum();
