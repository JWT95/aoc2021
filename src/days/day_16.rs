...
fn part_one(packet: &Packet) -> usize {
    packet.sub_packets.iter().map(|x| part_one(x)).sum::<usize>() + packet.version
}

fn evaluate_packet(packet: &Packet) -> usize {
    match packet.operator {
        0 => packet.sub_packets.iter().map(|x| evaluate_packet(x)).sum::<usize>(),
        1 => packet.sub_packets.iter().map(|x| evaluate_packet(x)).product::<usize>(),
        2 => packet.sub_packets.iter().map(|x| evaluate_packet(x)).min().unwrap(),
        3 => packet.sub_packets.iter().map(|x| evaluate_packet(x)).max().unwrap(),
        4 => packet.value,
        5 => if evaluate_packet(&packet.sub_packets[0]) > evaluate_packet(&packet.sub_packets[1]) {1} else {0},
        6 =>  if evaluate_packet(&packet.sub_packets[0]) < evaluate_packet(&packet.sub_packets[1]) {1} else {0},
        7 => if evaluate_packet(&packet.sub_packets[0]) == evaluate_packet(&packet.sub_packets[1]) {1} else {0},
        _ => unimplemented!()
    }
}
